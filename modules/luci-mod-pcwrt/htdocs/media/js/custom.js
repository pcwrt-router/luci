...
pcwrt = {
    ...
    fetch_data: function(url, data, cb, suppress_error) {
	$.ajax({
	    url: url,
	    data: data,
	    type: 'GET',
    ...
    submit_form: function($form, data, cb, contentType, omitSpinner, spinner_text, spinner_html, suppress_error) {
...
$('#upgrade-modal form .btn-success').on('click', function() {
    var $form = $(this).parent().parent();
    pcwrt.submit_form($form, $form.serialize(), function(resp) {
	$('#upgrade-modal').modal('hide');
	pcwrt.confirm_submit(null, $('#reboot'), msgs.reboot_title, msgs.upgrade_reboot, msgs.rebooting, '', function(r) {
	    $('<iframe src="'+r.reload_url+'"></iframe>').appendTo('#reloader');
	});
    });
});
