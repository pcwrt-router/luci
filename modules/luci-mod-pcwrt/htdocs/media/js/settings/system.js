...
$('[name=reset_override]').on('click', function(e) {
    if ($(this).prop('checked')) {
	$('[name=email_password_reset]').prop('checked', true);
    }
});

$('[name=email_password_reset]').on('click', function(e) {
    if ($('[name=reset_override]').prop('checked') && !$(this).prop('checked')) {
	pcwrt.show_message(window.msgs.warning_title, window.msgs.need_email_password_reset);
	$(this).prop('checked', true);
    }
});

$('a[href="#downgrade"]').on('click', function(e) {
    e.preventDefault();
    $('#downgrade-modal').modal('show');
});

$('#downgrade-modal form .btn-success').on('click', function() {
    var $form = $(this).parent().parent();
    pcwrt.submit_form($form, $form.serialize(), function(resp) {
	$('#downgrade-modal').modal('hide');
	pcwrt.confirm_submit(null, $('#reboot'), msgs.reboot_title, msgs.downgrade_reboot, msgs.rebooting, '', function(r) {
...
$(function() {
    ...
    $('#flash-modal button[type=button]').on('click', function(e) {
	e.preventDefault();
	var $form = $(this).parents('form');
	$.ajax({
	    url: $form.data('cancel'),
	    type: 'POST',
	    success: function(r) {
	    }
	});
