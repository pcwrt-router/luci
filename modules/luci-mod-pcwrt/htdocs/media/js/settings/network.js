...
function is_lan_ip(ip) {
    var lan = get_lan_if();
    return lan ? is_ip_on_network(ip, lan.ipaddr, lan.netmask) : false;
}

function get_new_ip(ipaddr, netmask, ip) {
    if (typeof(ipaddr) != 'string' || typeof(netmask) != 'string' || typeof(ip) != 'string') {
	return ip;
    }

    if (!pcwrt.is_valid_ipaddr(ipaddr) || !pcwrt.is_valid_ipaddr(netmask) || !pcwrt.is_valid_ipaddr(ip)) {
	return ip;
    }
    ...
    var ips = ip.split('.');
    var newip = [ ipaddrs[0] & masks[0], ipaddrs[1] & masks[1], ipaddrs[2] & masks[2], ipaddrs[3] & masks[3] ];
...
function merge_hosts() {
	...
	for (var i = 0; i < len; i++) {
		...
		fv.leases[i].orig_ipaddr = fv.leases[i].ipaddr;
...
function update_forward_hosts() {
    var ips = [];
    var ip = get_new_ip($('#ipaddr').val(), $('#netmask').val(), fv.ipaddr) + ' (Router)';
...
$(function() {
	...
	$("#vlan-ports").append('<div class="form-group col-md-2">'
	...
	var newip = is_lan_ip(fv.leases[idx].ipaddr) ?
		    get_new_ip($('#ipaddr').val(), $('#netmask').val(), fv.leases[idx].ipaddr)
		    : fv.leases[idx].ipaddr;
	$('#host-ip').val(newip);
    });
	...
	if (add_host_mac > 0) {
		...
		$('#host-ip').val($('#hosts tr:eq('+add_host_mac+') td:eq(1)').text().trim());
	...
	$('#hosts tr:gt(0)').each(function() {
	    ...
	    if (is_lan_ip(ip)) {
		$('td:eq(1)', $(this)).text(get_new_ip($('#ipaddr').val(), $('#netmask').val(), ip));
	    }
	});
	...
	$('#forwards tr:gt(0)').each(function() {
	    ...
	    if (is_lan_ip(ip)) {
		$('td:eq(3)', $(this)).text(iptxt.replace(ip, get_new_ip($('#ipaddr').val(), $('#netmask').val(), ip)));
	    }
	});
	...
	var vlans = [];
	$('#vlan-ports input[name*=port-]').each(function() {
	    vlans.push({
		port: $(this).attr('name').replace('port-',''),
		id: $(this).val()
	    });
	});
