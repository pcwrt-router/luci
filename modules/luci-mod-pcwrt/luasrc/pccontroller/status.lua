...
local function get_netaddrs(c, ntm)
    local netaddrs = {}
    c:foreach(network, 'interface', function(v)
	if v.type == 'bridge' then
	    local n = ntm.network(v['.name'])
		...
		netaddrs[#netaddrs + 1] = {
		    ...
		    ip = ipaddr,
		    mask = netmask,
		    network = luci.ip.IPv4(ipaddr, netmask):network(),
		}
	    end
	end
    end)
...
local function is_wifi_net_up(net, dev)
    if net:get('disabled') == '1' or not net:is_up() or net:channel() == nil then
	return false
    end
...
function get_assocs(c, ntm, devs, netaddrs)
	...
	for _, v in ipairs(netaddrs) do
	    if luci.ip.IPv4(ip, v.mask):network() == v.network then
		ok = true
		break
	    end
	end
...
function _get_data(c)
    ...
    local netaddrs = get_netaddrs(c, ntm)
