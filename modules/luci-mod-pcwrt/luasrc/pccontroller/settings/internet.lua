...
ordering = 10
display_name = i18n.translate('Internet')
...
local function get_wan_dev(c)
    local wandev
    local wanif = c:get(config, 'wan', 'ifname')
    c:foreach(config, 'device', function(d)
	if d.name == wanif then
	    wandev = d['.name']
	    return false
	end
    end)
...
function update()
	...
	if new_macaddr ~= nil and new_macaddr ~= old_macaddr then
	    ...
	    local wandev = get_wan_dev(c)
	    if wandev ~= nil then
		c:set(config, wandev, 'macaddr', new_macaddr)
	    end
	end
