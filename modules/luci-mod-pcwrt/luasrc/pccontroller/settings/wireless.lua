...
ordering = 30
display_name = i18n.translate('Wireless')
...
local function get_device_channel_width(c, devname)
    local hwtype = c:get(config, devname, 'type')
    local bw = c:get(config, devname, 'htmode')
...
local function set_device_channel_width(c, devname, bw)
    local hwtype = c:get(config, devname, 'type')
    local hwmode = c:get(config, devname, 'hwmode')
    local htmode = hwmode == '11a' and 'VHT' or 'HT'
    if bw == '2' then
...
function _get_data(c)
    ...
    local alwayson = '1'
    local schedule = nil
    local wifi_schedule = c:get_first(config, 'wifi-schedule')
    if wifi_schedule ~= nil then
	alwayson = c:get(config, wifi_schedule, 'alwayson')
	if alwayson == nil then
	    alwayson = '1'
	end
	schedule = c:get_list(config, wifi_schedule, 'schedule')
	if schedule ~= nil and type(schedule) == 'table' then
	    for i, v in ipairs(schedule) do
		schedule[i] = translate_time_slots(v)
	    end
	end
    end
	...
	if #ifaces > 0 then
		...
		local ifc = {}
		local vlan = get_vlan_params(iface.network)
		ifc.id = vlan.id
...
function _update(c, v)
    ...
    local schedule = {}
    local alwayson = v.alwayson
    if alwayson ~= '0' then alwayson = '1' end
    if v.schedule then
	local s = jsonc.parse(v.schedule)
	if s ~= nil then
	    for _, t in ipairs(s) do
		schedule[#schedule+1] = tostring(#schedule) .. ':' .. get_conf_timeslots(t)
	    end
	end
    end
    ...
    local ss
    if alwayson ~= 1 then
	c:delete_all(config, 'wifi-schedule')
	ss = c:section(config, 'wifi-schedule')
	if #schedule > 0 then
	    c:set_list(config, ss, 'schedule', schedule)
	end
    else
	ss = c:get_first(config, 'wifi-schedule')
    end

    if ss ~= nil then
	c:set(config, ss, 'alwayson', alwayson)
    end
	...
	if not dev.disabled and c:get(config, devname, 'type') ~= nil then
	    ...
	    if (dev.block_krack == '1') then
		c:set(config, devname, 'block_krack', '1')
	    else
		c:delete(config, devname, 'block_krack')
	    end
	...
	if vlan.name ~= 'lan' then
	    ...
	    if add_vlan then
		create_vlan_network(c, vlan.name, false, cfgs)
	    else
		local deleted, hosts_updated = delete_vlan_network(c, vlan.name, false, cfgs)
	    end
	end
    end
