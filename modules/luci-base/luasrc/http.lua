...
function urldecode_params(url, tbl, option)
	local parser, name
	local params = tbl or { }
	...
	parser = lhttp.urlencoded_parser(function (what, buffer, length)
		...
		elseif what == parser.NAME then
			name = lhttp.urldecode(buffer, option)
		elseif what == parser.VALUE and name then
		    local val = params[name]
		    if type(val) == "table" then
			val[#val+1] = lhttp.urldecode(buffer, option) or ""
		    elseif val ~= nil then
			params[name] = { val, lhttp.urldecode(buffer, option) or "" }
		    else
			params[name] = lhttp.urldecode(buffer, option) or ""
		    end
		end
